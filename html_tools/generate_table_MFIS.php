...
/** function generateTableMFIS ($labels, $result, $id)
 * \Funkcja generuje kod HTML tabelki na podstawie danych wejsciowych.
 *
 * Dewiacja od funkcji generateTable()
 * Pierwsza kolumna zawiera ukryty nr ID badania MFIS (ID_MFIS)
 * który jest w postaci formularza przesyłanego do pliku db_tools/delete_MFIS.php
...
function generateTableMFIS($labels, $result, $id){
	...
	for ($i=0;$i<$rowcount;$i++){
		...
		echo   '
					<td>
						<form id="delete_MFIS" action="../db_tools/delete_MFIS.php" method="post">
						<input type="hidden" name="ID_MFIS" value='.$row[0].'>
